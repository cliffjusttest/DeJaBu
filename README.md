...
2D 等距視角回合制 RPG，採用 **Godot 4 客戶端** + **Java Spring Boot 伺服器** 分離架構。多地圖探索、**明雷追擊／暗雷危險區域**遇敵、5×2 陣型隊伍戰鬥、元素克制、技能樹、怪物捕捉與夥伴養成、**醫館與死亡機制**（陣亡扣 EXP、傳送醫館；夥伴戰力耗盡需休養後至醫館治療）、NPC 對話樹、任務系統、背包與裝備系統（支援角色與夥伴裝備管理）、消耗道具系統（可疊加最多 999 個 / 堆，超過自動分堆）、**戰鬥中使用消耗道具**、**F1–F12 技能快捷鍵**（每個角色 / 夥伴獨立設定）、**Shift+F1–F12 道具快捷鍵**（所有人共用），以及可查看角色與所有夥伴完整數值的**狀態選單**（`C` 鍵）。
...
flowchart LR
    ...
    subgraph Server["伺服器 (Spring Boot)"]
        ...
        VisibleEnemy[VisibleEnemyService]
        DangerZone[DangerZoneService]
        Cooldown[EncounterCooldownService]
...
| **伺服器** | 帳號驗證、角色與技能持久化、地圖碰撞驗證、傳送、明雷追擊／暗雷遇敵、戰鬥結算、死亡／醫館機制、NPC 對話、任務進度、背包與裝備管理（全部為權威邏輯） |
...
│   │   ├── world/                  # 等距地圖、碰撞、傳送、NPC／明雷渲染
│   │   │   ├── visible_enemy_manager.gd
│   │   │   └── encounter_cooldown.gd
│   │   ├── game/
...
│   ├── data/maps/
│   │   ├── maps.json               # 216 張地圖、496 傳點、NPC、明雷、危險區
│   │   └── *.txt                   # ASCII 地圖（與 server 同步）
│   └── assets/
│
├── tools/
│   ├── generate_world_maps.py      # 從 WORLD_MAP.md 生成地圖 + V31 遷移
│   └── map_handcraft.py            # 名地手工微調（洛陽、赤壁等）
├── WORLD_MAP.md                    # 184–280 三國世界設計（216 圖、傳點表）
│
...
│       │   ├── service/            # Auth、Battle、Skill、Session、Companion、Map、Encounter、VisibleEnemy、DangerZone、EncounterCooldown、Npc、Quest、Equipment、Backpack
│       │   └── websocket/
│       ├── resources/
│       │   ├── application.yml
│       │   ├── maps/               # 伺服器端地圖（碰撞與傳送驗證）
...
- 角色欄位包含：名稱、外型、元素、七項能力值、地圖 ID、地圖座標、等級、技能點、當前 HP、**`story_era`**（個人主線章節，預設 `E1` 中平）。
...
| `defense` | 防禦 | 百分比減傷（每 1 點 −0.1%）；防禦姿態有效防禦 ×2；減傷上限 75% |
| `spirit` | 精神 | 最大 MP = 精神 × 5 |
| `luck` | 幸運 | 暴擊率（攻方幸運 − 守方幸運，每 1 點差 +1%）、逃跑成功率（依敵方最高幸運差）、捕捉成功率 |
| `agility` | 敏捷 | 決定行動順序（數值越高越先行動）；連擊配對判斷；逃跑成功率（依敵方最高敏捷差） |

傷害與減傷公式（伺服器 `CharacterStats` 為準）：

- 最大 HP：`體力 × 20`；最大 MP：`精神 × 5`
- 普攻：`max(1, 武力)`，再套用元素克制
- 減傷：`min(75%, 有效防禦 × 0.1%)`（防禦姿態有效防禦 ×2），最低仍受 1 點傷害
- 暴擊：機率 = `max(0, 攻方幸運 − 守方幸運)%`，傷害 ×2
- 逃跑：`Clamp(0.35 + (行動人敏捷 − 敵方最高敏捷) × 1% + (行動人幸運 − 敵方最高幸運) × 1% + (行動人等級 − 敵方最高等級) × 10%, 10%, 95%)`

### 地圖與探索

完整世界設計見 **[WORLD_MAP.md](WORLD_MAP.md)**（184–280 AD、13 州、216 張地圖、223 組雙向傳點）。地圖以 ASCII 文字檔定義，客戶端負責渲染，伺服器負責碰撞與傳送驗證。兩端各有一份地圖檔，修改時需保持同步：

- 客戶端：`client/data/maps/`
- 伺服器：`server/src/main/resources/maps/`
- 重新生成：`python3 tools/generate_world_maps.py`（名地手工佈局見 `tools/map_handcraft.py`）

地圖清單與傳送點由 `maps.json` 設定（節錄）：

```json
{
  "defaultMap": "xuchang",
  "maps": {
    "xuchang": { "file": "xuchang.txt", "name": "許縣" },
    "yingchuan_suburb": {
      "file": "yingchuan_suburb.txt",
      "name": "潁川郊野",
      "maxVisibleEnemies": 2,
      "maxDarkEnemies": 3,
      "visibleEnemies": [
        { "id": "yingchuan_wolf_1", "templateId": "wild_wolf", "x": 6, "y": 14, "chaseRange": 5, "loseRange": 8 }
      ],
      "dangerZones": [
        { "id": "yingchuan_suburb_wild", "rect": { "x1": 2, "y1": 2, "x2": 25, "y2": 17 } }
      ]
    }
  },
  "teleports": {
    "xuchang:1,1": { "map": "road_luoyang_xuchang", "x": 34, "y": 5 },
    "yingchuan_suburb:26,18": { "map": "yingchuan", "x": 1, "y": 1 }
  },
  "npcs": {
    "xuchang": [
      { "id": "hospital_xuchang", "name": "許縣醫廬", "x": 8, "y": 8, "spriteKey": "healer" },
      { "id": "xuchang_elder", "name": "鄉老", "x": 12, "y": 10, "spriteKey": "elder" }
    ]
  }
}
```
...
| 地圖 ID | 名稱 | 說明 |
|---------|------|------|
| `xuchang` | 許縣 | **預設出生點** (8, 8)；鄉老給予「初試身手」 |
| `yingchuan_suburb` | 潁川郊野 | 2 隻明雷野狼 + 暗雷區；斥候給予「黑霧調查」 |
| `luoyang` | 洛陽 | 帝都（手工佈局）；太醫署 |
| `chibi` | 赤壁 | 長江渡口（手工佈局） |
| `battlefield_guandu` | 官渡 | 官渡之戰戰場（手工佈局） |

其餘 200+ 張地圖與完整傳點表見 [WORLD_MAP.md](WORLD_MAP.md) §4–§5。
...
**傳送**：踩到 `maps.json` 設定的傳送座標時，`MOVE_OK` 帶 `mapChanged: true`，伺服器更新 DB 中的地圖 ID 與座標，客戶端重新載入地圖。傳送點不觸發戰鬥。

**明雷（可見敵人）**

- 每張地圖在 `maps.json` 的 `visibleEnemies` 設定出生點（彼此間距 ≥ 4 格）
- 明雷會主動追擊追擊範圍內最近的玩家（預設追 5 格、脫離 8 格），同一時間只追一人
- 目標進入戰鬥時改追其他可追擊玩家，並回到出生點
- 接觸玩家（相鄰或同格）觸發戰鬥；`MOVE_OK` 帶 `encounter: true`、`visibleEnemyId`、`wildMonsters`
- 每張地圖可設定 `maxVisibleEnemies`（明雷戰鬥最大敵人數，預設 1）與 `maxDarkEnemies`（暗雷戰鬥最大敵人數，預設 3）；實際遭遇數為 1～上限的隨機值
- 客戶端 `VisibleEnemyManager` 渲染明雷並同步位置；收到 `VISIBLE_ENEMY_UPDATE` 廣播時更新

**暗雷（危險區域）**

- 危險區域在 `dangerZones` 設定（支援 `rect` 矩形或 `cells` 格子列表）
- 進入時提示「這裡充滿危險氣息...」（`dangerZoneEntered: true`）
- 區域內每走 1 步 +1 危險值、每場戰鬥 +3；離開區域重置
- 遇敵機率：`min(80%, 危險值 × 3%)`；觸發時 `MOVE_OK` 帶 `fromDangerZone: true`

**戰鬥後冷卻**（以隊長為準，組隊時全隊共用）

| 冷卻 | 一般戰鬥 | 危險區域戰鬥 |
|------|----------|--------------|
| 對應明雷屏蔽（客戶端隱藏 + 伺服器阻擋） | 20 秒 | 20 秒 |
| 明雷追擊 | 5 秒 | 5 秒 |
| 明雷戰鬥 | 5 秒 | 5 秒 |
| 暗雷戰鬥 | 5 秒 | 3 秒 |
...
**組隊時**：僅**隊長**可與 NPC 對話；隊員透過 `PARTY_DIALOGUE_SYNC` 旁聽（唯讀）。NPC 起始節點依**隊長**的 `story_era` 與任務狀態解析。詳見 [GAME_MECHANICS.md §7.5](GAME_MECHANICS.md#75-主線章節與個人時間線)。

**互動流程**

```
靠近 NPC（距離 ≤ 1 格）→ 按 F（組隊時僅隊長）
  → 伺服器依「有效故事玩家」（組隊 = 隊長）的任務狀態決定起始節點
    ├─ 有任務可領取  → quest_complete 節點
    ├─ 任務進行中    → quest_already 節點
    └─ 無任務（預設）→ root 節點
  → 隊長：NPC_INTERACT_OK；隊員：PARTY_DIALOGUE_SYNC（observer: true）
  → 隊長點選選項 → DIALOGUE_CHOICE
  → 伺服器：執行副作用（接受任務 / 領取報酬）→ 同步下一節點給全隊
...
| NPC ID | 名稱 | 地圖 | 角色 | 備註 |
|--------|------|------|------|------|
| `xuchang_elder` | 鄉老 | 許縣 | 任務 | E1 主線「初試身手」 |
| `hospital_xuchang` | 許縣醫廬 | 許縣 | 醫館 | 可對話「治療夥伴」 |
| `yingchuan_scout` | 斥候 | 潁川郊野 | 任務／商店 | E1 主線「潁川賊營」 |
| `hospital_luoyang` | 太醫署 | 洛陽 | 醫館 | 帝都醫館 |

13 座醫館分佈於洛陽、許縣、長安、潁川等州治，完整列表見 [WORLD_MAP.md](WORLD_MAP.md) §9 與 `V31__world_map.sql`。

### 任務與主線章節

任務定義於 `quests` 表，玩家進度存於 `player_quests` 表。每位玩家另有 **`users.story_era`**（E1–E8）表示個人主線章節；**不做全服統一推進**。完整時間線規則見 [GAME_MECHANICS.md §7.5](GAME_MECHANICS.md#75-主線章節與個人時間線)。
...
| 欄位 | 說明 |
|------|------|
| `required_era` | 可接取的最低個人時代（如 `E1`） |
| `prerequisite_quest_id` | 須先完成的任務 ID |
| `unlocks_era` | 領獎後可推進的下一章節（可選） |

**任務狀態流程（KILL 任務）**

```
接受任務（IN_PROGRESS, progress=0）— 組隊時僅隊長可接
  → 每次戰鬥勝利，伺服器對「每位已接此任務的隊員」更新 progress
  → progress ≥ requiredCount → 任務「可領取」
  → 隊長與 NPC 對話 → quest_complete 節點
  → 隊長選「領取報酬」→ 已接且達標的全隊成員各得 EXP + 技能點，status → COMPLETED
  → 若設 unlocks_era → 更新該員 users.story_era
```

**E1 現有主線任務**

| ID | 名稱 | 前置 | 目標 | 數量 | 給予者 | 報酬 |
|----|------|------|------|------|--------|------|
| 1 | 初試身手 | — | wild_wolf | 3 | 鄉老 | 60 EXP + 1 技能點 |
| 2 | 潁川賊營 | 任務 1 | shadow_wisp | 2 | 斥候 | 80 EXP + 1 技能點 |

未接任務的隊員可跟隊戰鬥，但**不會**獲得任務進度。

戰鬥勝利時，`BATTLE_RESULT` 帶有各員個人化的 `questProgress` 陣列。
...
- **單人**：玩家 **slot 7**（後排中央）；出戰夥伴依隊伍索引佔 slot **2、1、3、0、4**，最多 **5 名**。
- **組隊**：隊長 slot **7**，其他玩家依序 **6、8、5、9**；各玩家的夥伴在其角色正前方（角色格 - 5，例如角色 7 → 夥伴 2，角色 6 → 夥伴 1）。
- **明雷戰鬥**：依該明雷的 `templateId` 生成敵人，數量 1～該地圖 `maxVisibleEnemies`（預設 1）。
- **暗雷戰鬥**：從該地圖可生成的暗雷模板中隨機選取，數量 1～該地圖 `maxDarkEnemies`（預設 3）。
- 敵人等級：依怪物模板 `min_level`～`max_level` 隨機決定。
...
| 敏捷相近 | 相鄰候選單位的敏捷差距 ≤ 15（門檻值） |
| 行動可連擊 | 普攻，或技能的「可連擊」(`combo_eligible`) 欄位為 `true`；同組可混用普攻與技能 |
| 機率 | 每次配對各自獨立判定：`Clamp(0.5 + (發動方平均等級 − 對方平均等級) × 0.05 + max(0.1, 配對候選單位幸運 × 0.001), 0, 0.95)` |
...
1. 取敏捷最高的尚未分配單位為**候選先手**，與下一個尚未分配單位**嘗試配對**。
   ...
   - 差距 ≤ 15，依連擊機率公式判定（見上表「機率」欄）：
     - **失敗** → 先手單獨行動；後手從下一個候選者重新判定（不因本次失敗而跳過）。
     - **成功** → 兩人形成連擊組；繼續嘗試將下一個候選者**延伸**加入：
       - 差距 > 15 或候選者耗盡 → 停止延伸。
       - 再依連擊機率公式判定：**成功** → 加入本組，繼續；**失敗** → 停止延伸，該候選者以剩餘單位為對象**重新開始**新一輪配對。
...
|------|------|--------|------|
| 攻擊 | `1` → 點敵 | 全員 | 單體普攻 |
| 防禦 | `2` | 全員 | 本回合有效防禦 ×2（等同雙倍防禦減傷率） |
| 逃跑 | `3` | 全員 | 依行動者敏捷／幸運／等級與敵方最高值差判定；執行時成功則立即結束戰鬥 |
...
**敵方 AI**：55% 機率使用有效技能（含治療／復活／輔助隊友技能，但會判定目標是否有效；無有效目標時改試其他技能或普攻），否則普攻；攻擊對象為隨機存活我方單位。
...
- 與各地 **醫館 NPC**（`hospital_*`）對話，選「治療夥伴」
...
| 起始等級 | 1 |
| 起始技能點 | 1 |
| 每隻怪物 EXP | `5 × 怪物等級 × (1 / (1 + e^((單位等級 - 怪物等級) / 10)))`；單位等級與怪物差距 ≥ 20 時為 0 |
...
```
怪物 EXP = 5 × 怪物等級 × (1 / (1 + e^((單位等級 - 怪物等級) / 10)))
（單位等級與怪物等級差距 ≥ 20 時，該單位 EXP = 0）
共享部分 = floor(怪物 EXP × 0.75)   → 所有我方單位（含玩家與夥伴）各自獲得
擊殺獎勵 = 怪物 EXP − 共享部分      → 僅發給擊殺該怪物的單位
```

- **一般擊殺**：擊殺者獲得「共享部分 + 擊殺獎勵」，其他人只獲得共享部分。
- **連擊擊殺**：所有參與連擊的成員各自都能取得完整擊殺獎勵（不分攤，人數不限），並**額外**獲得怪物 EXP 的 **10%**（非連擊擊殺者無此加成）。
- **範例**（怪物 EXP = 100）：共享 = 75，擊殺獎勵 = 25。連擊擊殺者各得 75 + 25 + 10 = **110**；一般擊殺者得 75 + 25 = **100**；其餘人各得 **75**。
...
| 模板 ID | 名稱 | 元素 | 可捕捉 | 等級範圍 | 明雷 | 暗雷 | 生成地圖 | 預設技能 |
|---------|------|------|--------|----------|------|------|----------|----------|
| `wild_wolf` | 野狼 | 風 | ✅ | 1～10 | ✅ | ✅ | 多州野原／郊野 | 基礎劍術、重劈 |
| `shadow_wisp` | 幽影 | 無 | ✅ | 3～12 | ✅ | ✅ | 黃巾營寨等 | 基礎法術、火球術 |

**模板生成設定（`monster_templates`）**

| 欄位 | 說明 |
|------|------|
| `min_level` / `max_level` | 野外遭遇時的怪物等級範圍 |
| `visible_spawn` | 是否可作為明雷生成（須在 `maps.json` 的 `visibleEnemies` 指定） |
| `dark_spawn` | 是否可作為暗雷隨機生成 |
| `monster_template_spawn_maps` | 允許生成的地圖 ID（多對多關聯表） |

伺服器啟動時會驗證：`maps.json` 中的明雷模板必須允許在該地圖作為明雷生成；設有危險區的地圖必須至少有一個可生成的暗雷模板。
...
- **僅隊長**可與 NPC 對話（`F`）；隊員旁聽 `PARTY_DIALOGUE_SYNC`
- 隊長遭遇野生怪物時，全隊一起進入同一場戰鬥
- 組隊中**有效主線時間線**為隊長的 `story_era`；狀態列顯示「中平（隊長時間線）」等提示
...
- 全隊共用一場戰鬥；我方陣型：隊長 slot 7 + 夥伴 slot 2，其餘成員依序佔 slot 6/8/5/9，夥伴各在對應角色前方（1/3/0/4）
- 每位玩家**只能**為自己的角色與夥伴指定行動（單位帶 `ownerUserId`）
- **所有玩家**都為己方全部單位指定完畢後，才進入執行階段（依敏捷交叉行動，規則同單人戰鬥）
- 勝利時每位隊員各自獲得 EXP、金幣與掉落
- **任務擊殺進度**：僅**已接該 KILL 任務**的成員累積；未接者可參戰但無進度
- **任務領獎**：隊長對話領取時，全隊已接且達標者一併結算
...
- **VisibleEnemyManager** — 渲染明雷 sprite、同步伺服器位置、依 `EncounterCooldown` 屏蔽冷卻中的明雷
- **EncounterCooldown** — 追蹤戰鬥後冷卻與明雷屏蔽（客戶端本地計時，並同步伺服器 snapshot）
...
啟動 → REST 登入／註冊 →（首次）REST 創建角色
  → WebSocket 登入 → 探索模式（WASD / 點擊移動）
    ...
    → 明雷追擊接觸／危險區暗雷 → 戰鬥模式（指定所有單位行動 → 依敏捷交叉執行）
...
| C→S | `NPC_INTERACT` | 與 NPC 開始對話（**組隊時僅隊長**） |
...
| C→S | `PING` | 心跳 |
| S→C | `LOGIN_OK` | 登入成功，回傳角色座標、地圖、元素、能力、HP、`otherPlayers`、`party`、`visibleEnemies`、冷卻 snapshot 等 |
| S→C | `MOVE_OK` | 移動確認，可能帶 `encounter`、`visibleEnemyId`、`fromDangerZone`、`wildMonsters`、`visibleEnemies`、`inDangerZone`、`dangerValue`、`dangerZoneEntered`、冷卻欄位；換圖時另帶 `otherPlayers` |
| S→C | `VISIBLE_ENEMY_UPDATE` | 同地圖明雷位置廣播，帶 `mapId`、`visibleEnemies` |
...
| S→C | `PARTY_SYNC` | 隊員跟隨隊長移動／傳送，帶 `x`、`y`、`mapId`、`mapChanged`、`encounter`、`visibleEnemyId`、冷卻欄位 |
| S→C | `PARTY_DIALOGUE_SYNC` | 組隊時隊員同步隊長 NPC 對話（`observer: true`，無選項） |
| S→C | `PONG` | 心跳回應 |
| S→C | `ERROR` | 錯誤訊息 |

`BATTLE_ACTION` 額外欄位：`actorId`（行動單位）、`targetId`（目標格）、`skillId`（技能施放時）、`itemId`（道具使用時）。

`BATTLE_RESULT` 當 `roundExecuted: false` 時僅更新 `battle.plannedActorIds`；`roundExecuted: true` 時包含完整 `attackEvents`、`message`、`battleOver`，勝利時附帶 `expGained`、`playerLevel`、`questProgress: [{questName, progress, requiredCount, readyToClaim}]`，若有夥伴獲得 EXP 則附帶 `companionExpResults: [{companionId, name, expGained, previousLevel, newLevel, leveledUp}]`。戰鬥結束（`battleOver` 或 `escaped`）時附帶 `visibleEnemyId`（明雷來源）、`fromDangerZone`（是否暗雷）。戰鬥失敗或逃跑且有人 HP = 0 時附帶 `deathResult`（單人）或 `deathOutcomes`（組隊，依 `playerId` 個人化），欄位含 `playerDied`、`expLost`、`playerExp`、`teleportMapId`、`teleportX`、`teleportY`、`companionResults: [{companionId, nickname, expLost, incapacitatedMinutes}]` 等。組隊戰鬥勝利時另附 `playerExpResults`、`lootResults`（每位隊員一筆，含 `playerId`），客戶端會依自身 `playerId` 展開為單人欄位。

`MOVE_OK`／`LOGIN_OK`／`PARTY_SYNC` 冷卻欄位：`noVisibleEncounterMs`、`chaseCooldownMs`、`darkEncounterCooldownMs`、`maskedVisibleEnemies: { enemyId: remainingMs }`。

`party` 物件欄位：`inParty`、`isLeader`、`leaderId`、`maxSize`、`maxCompanionsPerPlayer`、`members: [{playerId, playerName, playerLevel, isLeader}]`、**`playerStoryEra`、`effectiveStoryEra`、`effectiveStoryEraName`**、（選填）`pendingInviteFrom`、`pendingInviteFromName`。

`battle` 快照在組隊戰鬥時帶 `multiplayer: true`；我方單位帶 `ownerUserId`，客戶端僅能操作 `ownerUserId` 等於自己的單位。

`LOGIN_OK` 含 `playerStoryEra`、`effectiveStoryEra`、`effectiveStoryEraName`；`party` 物件亦含相同欄位。

`NPC_INTERACT_OK` 對話進行中欄位：`npcId`、`npcName`、`nodeKey`、`text`、`choices: [{index, text}]`、`observer`（隊員旁聽為 `true`）；對話結束（`finished: true`）時帶 `message` 與 `questRewards: [{questName, expGained, skillPointsGained}]`（各員僅含自己的獎勵）；選「開啟商店」時另帶 `openShop: true`；醫館治療時另帶 `hospitalRevive: true/false`。`PARTY_DIALOGUE_SYNC` 欄位相同。
...
- **MapService** — 地圖載入、可走性驗證、傳送點解析、明雷出生點與危險區域載入
- **EncounterService** — 野外遭遇生成與暫存（明雷／暗雷）
- **VisibleEnemyService** — 明雷追擊邏輯、位置更新、接觸判定
- **DangerZoneService** — 危險值累計、暗雷遇敵機率
- **EncounterCooldownService** — 戰鬥後冷卻（明雷屏蔽、追擊、明雷／暗雷戰鬥）
...
- **StoryContextService** — 解析有效主線時間線（組隊 = 隊長 `story_era`）
- **NpcService** — NPC 互動、對話樹、依**有效故事玩家**任務狀態決定起始節點、組隊對話同步、任務接受／報酬
- **QuestService** — 任務接受（era／前置檢查）、擊殺進度、報酬發放、`unlocks_era` 章節推進
...
| 表 | 用途 |
|----|------|
| `users` | 帳號、角色、能力值、等級、EXP、技能點、當前 HP、座標、地圖 ID、**`story_era`** |
| `auth_tokens` | 登入 session token |
| `skills` | 技能定義（含 `combo_eligible` 欄位） |
| `skill_prerequisites` | 技能前置關係 |
| `user_skills` | 角色已學技能與等級 |
| `monster_templates` | 野外怪物模板（含等級範圍、明雷／暗雷生成開關） |
| `monster_template_spawn_maps` | 怪物模板可生成的地圖 |
...
| `quests` | 任務定義（類型、目標、報酬、**`required_era`**、**`prerequisite_quest_id`**、**`unlocks_era`**） |
...
- 遇敵規則由伺服器判定：**明雷**追擊接觸、`VisibleEnemyService`；**暗雷**危險區累計，`DangerZoneService`；戰鬥後冷卻由 `EncounterCooldownService` 管理（以隊長為準）
...
- 新增任務：在 migration 插入 `quests`（設 `required_era`、前置、`unlocks_era`）與 `dialogue_nodes`；遵循 [GAME_MECHANICS.md §7.5](GAME_MECHANICS.md#75-主線章節與個人時間線) 擴充指引
- 地圖修改：同時編輯客戶端與伺服器兩份 `.txt`，並在 `maps.json` 設定傳送點、明雷（`visibleEnemies`）、危險區（`dangerZones`）後重啟
