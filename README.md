...
- 角色欄位包含：名稱、外型、元素、六項能力值、地圖 ID、地圖座標、等級、技能點、當前 HP。
...
創角時從 0 點基底出發，分配 **10 點**自由點數至六項能力（每項 0–99）：
...
2. **執行階段（我方）**：我方單位依單位 ID 由小到大執行各自指定的行動；若行動目標在執行前已被擊倒，自動跳過並提示。
3. **執行階段（敵方）**：每隻存活的敵方怪物**各自行動一次**（普攻或技能），順序依序輪完。
4. 技能冷卻在我方全員行動後遞減 1 回合。
5. 重複直到一方全滅、成功逃跑，或捕捉導致敵方清空。
...
| 結果 | 條件 | 效果 |
|------|------|------|
| 勝利 | 敵方全滅 | 獲得 EXP（各敵等級 × 5 加總）；達標自動升級 |
