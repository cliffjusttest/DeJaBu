...
2D 等距視角回合制 RPG，採用 **Godot 4 客戶端** + **Java Spring Boot 伺服器** 分離架構。多地圖探索、隨機遇敵、5×2 陣型隊伍戰鬥、元素克制、技能樹、怪物捕捉與夥伴養成、NPC 對話樹、任務系統、裝備系統。
...
flowchart LR
    ...
    subgraph Server["伺服器 (Spring Boot)"]
        REST[Auth / Character / Skills / Companions / Equipment API]
...
| 層級 | 職責 |
|------|------|
| **客戶端** | 等距地圖渲染、輸入、動畫、UI 呈現（登入、創角、技能樹、夥伴、戰鬥、對話、任務日誌、裝備管理） |
| **伺服器** | 帳號驗證、角色與技能持久化、地圖碰撞驗證、傳送、遇敵、戰鬥結算、NPC 對話、任務進度、裝備管理（全部為權威邏輯） |
...
│   └── src/main/
│       ├── java/com/dejebu/
│       │   ├── controller/         # Auth、Character、Skill、Companion、Equipment REST API
│       │   ├── entity/             # User、Skill、AuthToken、Companion、Npc、Quest、Item、UserEquipment 等
│       │   ├── game/               # Element、EquipmentSlot、BattleFormation、SkillCombatCalculator 等
│       │   ├── service/            # Auth、Battle、Skill、Session、Companion、Map、Encounter、Npc、Quest、Equipment
│       │   └── websocket/
│       ├── resources/
│       │   ├── application.yml
│       │   ├── maps/               # 伺服器端地圖（碰撞與傳送驗證）
...
裝備定義於 `items` 表，玩家已裝備的道具存於 `user_equipment` 表（主鍵：`user_id + slot`，每個部位最多一件）。
...
每件裝備可提升七項能力值的任意組合（`bonus_might`、`bonus_intelligence`、`bonus_vitality`、`bonus_defense`、`bonus_spirit`、`bonus_luck`、`bonus_agility`），加成上限不超過各能力值上限 99。

#### 裝備對戰鬥的影響

進入戰鬥時，伺服器在 `GameWebSocketHandler` 中將玩家的**基礎能力值**加上所有已裝備道具的**加成合計**，以最終能力值建立 `BattleUnit`，並傳入 `BattleService`。裝備加成對以下所有戰鬥計算均有效：普攻傷害、減傷、技能傷害、暴擊率、逃跑率。
...
按畫面「裝備」按鈕或 `E` 鍵開啟裝備面板：
- **左欄**：目前九個部位的裝備狀態，及每件裝備的能力加成，可個別「卸下」。
- **右欄**：依等級篩選後的所有可用道具，按部位分組顯示，點「裝備」即替換對應部位（同部位自動卸下舊裝備）。

#### REST API（裝備）

| 方法 | 路徑 | 說明 |
|------|------|------|
| `POST` | `/api/equipment/status` | 取得目前裝備狀態與可用道具清單（需 token） |
| `POST` | `/api/equipment/equip` | 裝備指定道具（需 token、`itemId`） |
| `POST` | `/api/equipment/unequip` | 卸下指定部位（需 token、`slot`） |
...
- **EquipmentService** — 裝備查詢、裝備道具（自動替換同部位舊裝備）、卸下、計算能力加成合計
...
| `items` | 裝備道具定義（部位、需求等級、七項能力加成） |
| `user_equipment` | 玩家目前裝備（user_id + slot 為主鍵，指向 items） |
...
- Schema 變更：新增 Flyway migration 至 `server/src/main/resources/db/migration/`（目前最新為 V20）
- 新增道具：在 `V20__equipment.sql`（或新 migration）的 `items` 表新增記錄，`slot` 值須對應 `EquipmentSlot` 枚舉
