...
2D 等距視角回合制 RPG，採用 **Godot 4 客戶端** + **Java Spring Boot 伺服器** 分離架構。多地圖探索、隨機遇敵、5×2 陣型隊伍戰鬥、元素克制、技能樹、怪物捕捉與夥伴養成、NPC 對話樹、任務系統、背包與裝備系統（支援角色與夥伴裝備管理）、消耗道具系統（可疊加最多 999 個 / 堆，超過自動分堆）、**戰鬥中使用消耗道具**、**F1–F12 技能快捷鍵**（每個角色 / 夥伴獨立設定）、**Shift+F1–F12 道具快捷鍵**（所有人共用），以及可查看角色與所有夥伴完整數值的**狀態選單**（`C` 鍵）。
