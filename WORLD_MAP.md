...
> **設計狀態**：完整節點與傳點規劃（**已實作** ASCII 地圖與 `maps.json`）；**個人主線時間線**（`story_era`）已實作，見 §8.4  
...
| **全時間線** | 同一套地圖覆蓋 184–280；`eraTags` 標記主要活躍時期；**玩家個人進度**以 `users.story_era` 實作（見 [GAME_MECHANICS.md](GAME_MECHANICS.md) §7.5） |
...
| 出生座標 | `(8, 8)` | 城內安全區；許縣醫廬同格 |
| 創角 `story_era` | `E1`（中平） | 從黃巾之亂／鄉勇入伍體驗開始 |
| 開局敘事 | 鄉老徵募鄉勇 | 見 `V32__story_era_and_e1_opening.sql` |

**E1 新手主線動線**

```
許縣 (xuchang) ─ 鄉老「初試身手」
    → 許昌外郭 → 潁川郊野 (yingchuan_suburb) 野狼
    → 潁川斥候「潁川賊營」
    → 潁川黃巾營 (rebel_camp_yingchuan) 黑霧精靈
    → （後續 E1 章節／E2 解鎖待擴充）
```

**替代出生點（創角可選 — 規劃中，尚未實作 UI）**
...
本文件 §7 定義 **184–280 全時間線的地理與史實節點**；執行時採「**地理永恆、故事個人化**」：

| 層次 | 資料 | 說明 |
|------|------|------|
| **共享地理** | 216 張 `map_id`、496 傳點 | 所有玩家同一套世界；不因有人通關赤壁而移除地圖 |
| **個人時間** | `users.story_era`（E1–E8） | 每位玩家自己的主線章節；預設 E1 |
| **個人進度** | `player_quests` | 任務接受／擊殺／領獎互不影響 |
| **地圖 eraTags** | `maps.json` 可選欄位 | 標記該地**史實上**活躍的時代（如 `julu`: E1）；供設計對照與日後遭遇過濾 |
| **組隊有效時間** | 隊長的 `story_era` | 組隊中 NPC 對話與主線節點以隊長為準 |

**為何不用「全服統一日曆」**

- 玩家 A 可能已在 E5（三國鼎立），玩家 B 仍在 E1（黃巾）— 若強制全服 E5，B 無法體驗開局
- 若強制全服 E1，A 的主線無法推進
- 解法：**不推進伺服器時鐘**；每人靠任務推進自己的 `story_era`；組隊時暫跟隊長

**設計新章節時（與 WORLD_MAP 的對照）**

1. 在 §7.2 查事件 ↔ 地圖 ↔ `eraTags`
2. 新增 `quests.required_era` 對應該章節（如 E4 赤壁線 → `required_era = 'E4'`）
3. 地圖無須複製多份；同一 `battlefield_chibi` 可服務所有 E4+ 玩家
4. 戰場類高難副本日後可改為「任務解鎖實例」，仍使用同一 `map_id` 的 ASCII 檔

完整互動規則（組隊對話、擊殺進度、領獎）見 **[GAME_MECHANICS.md §7.5](GAME_MECHANICS.md#75-主線章節與個人時間線)**。
...
| 2026-06-24 | 1.1 | §8.1 E1 鄉勇入伍開局已實作；新增 §8.4 個人時間線與共享地圖（對照 GAME_MECHANICS §7.5） |
